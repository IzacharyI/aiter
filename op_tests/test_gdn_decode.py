...
def bench_single(args, target_func, ref_func_local, warmup, niters):
    ...
    maxdiff_out = (outputs[0] - ref_outputs[0]).abs().max().item()
    maxdiff_state = (inouts[-2] - ref_inouts[-2]).abs().max().item()
    ok = maxdiff_out < 0.05 and maxdiff_state < 0.05
    print(f"maxdiff_out={maxdiff_out:.6f} maxdiff_state={maxdiff_state:.6f}", flush=True)
    if ok:
        print("VALIDATION_OK", flush=True)
    else:
        print("VALIDATION_FAIL", flush=True)
