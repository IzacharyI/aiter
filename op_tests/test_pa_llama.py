...
def pertoken_quant_kvcache_fp4(
    ...
    Returns:
        k_quant: [num_blocks, num_heads, head_size // (2*x), block_size, x] (packed fp4x2)
        k_scale: [num_heads, total_tokens]
        v_quant: [num_blocks, num_heads, head_size // 2, block_size] (packed fp4x2)
    ...
    # Compute per-token scale: scale = max(abs(x)) / FP4_MAX
    # Shape: [num_blocks, num_heads, block_size, 1]
    k_amax = key_cache_permute.abs().amax(dim=-1, keepdim=True).clamp(min=1e-12)
    k_scale_values = k_amax / FP4_MAX
    del k_amax
    
    v_amax = value_cache_permute.abs().amax(dim=-1, keepdim=True).clamp(min=1e-12)
    v_scale_values = v_amax / FP4_MAX
    del v_amax
    
    def quantize_to_fp4_chunked(x, scale, chunk_size=50000):
        """Quantize float values to FP4 E2M1 (returns uint8 indices 0-15)
        Memory-efficient chunked version to avoid FPE with large tensors."""
        num_blocks = x.shape[0]
        result = torch.empty(x.shape, dtype=torch.uint8, device=x.device)
        ...
        for start in range(0, num_blocks, chunk_size):
            ...
            # Quantize using comparisons (more stable than bucketize for bf16)
            idx = torch.zeros_like(abs_scaled, dtype=torch.uint8)
            idx = torch.where(abs_scaled >= 0.25, torch.tensor(1, dtype=torch.uint8, device=x.device), idx)
            idx = torch.where(abs_scaled >= 0.75, torch.tensor(2, dtype=torch.uint8, device=x.device), idx)
            idx = torch.where(abs_scaled >= 1.25, torch.tensor(3, dtype=torch.uint8, device=x.device), idx)
            idx = torch.where(abs_scaled >= 1.75, torch.tensor(4, dtype=torch.uint8, device=x.device), idx)
            idx = torch.where(abs_scaled >= 2.5, torch.tensor(5, dtype=torch.uint8, device=x.device), idx)
            idx = torch.where(abs_scaled >= 3.5, torch.tensor(6, dtype=torch.uint8, device=x.device), idx)
            idx = torch.where(abs_scaled >= 5.0, torch.tensor(7, dtype=torch.uint8, device=x.device), idx)
            del abs_scaled
            
            result[start:end] = sign + idx
            del sign, idx
        
        return result
    
    # Quantize K
    k_fp4_indices = quantize_to_fp4_chunked(key_cache_permute, k_scale_values)
    del key_cache_permute
    torch.cuda.empty_cache()
    
    # Quantize V
    v_fp4_indices = quantize_to_fp4_chunked(value_cache_permute, v_scale_values)
    del value_cache_permute
    torch.cuda.empty_cache()
    
    # UNPACKED format: 1 FP4 value per byte (lower 4 bits only)
    # This matches the kernel's expectation for FP4 data
    k_unpacked = k_fp4_indices  # [num_blocks, num_heads, block_size, head_dim]
    v_unpacked = v_fp4_indices
    
    # Reshape k_unpacked to vLLM format: [num_blocks, num_heads, head_size//x, block_size, x]
    # For unpacked FP4: x = 16 bytes, each 16 bytes contains 16 FP4 values
    quant_x = 16
    k_quant = (
        k_unpacked.view(num_blocks, num_heads, block_size, head_dim // quant_x, quant_x)
        .permute(0, 1, 3, 2, 4)
        .contiguous()
    )
    # v_quant: [num_blocks, num_heads, head_dim, block_size]
    v_quant = (
        v_unpacked.view(num_blocks, num_heads, block_size, head_dim)
        .permute(0, 1, 3, 2)
        .contiguous()
    )
...
def dequant_fp4_to_float(packed: torch.Tensor, scale: torch.Tensor, target_dtype: torch.dtype = None) -> torch.Tensor:
    """
    Dequantize FP4 packed values back to float for accuracy verification.
    Wrapper that uses chunked processing for large tensors.
    
    Args:
        packed: [num_blocks, num_heads, block_size, dim//2] uint8 packed fp4x2 values
        scale: [num_blocks, num_heads, block_size, 1] per-token scale values
    """
    # Use chunked processing for large tensors (> 100K blocks)
    if packed.shape[0] > 100000:
        return dequant_fp4_packed_to_float_chunked(packed, scale, target_dtype)
    
    if target_dtype is None:
        target_dtype = torch.bfloat16
        
    # FP4 E2M1 value table (use target dtype to save memory)
    fp4_values = torch.tensor([0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0,
                               -0.0, -0.5, -1.0, -1.5, -2.0, -3.0, -4.0, -6.0],
                              dtype=target_dtype, device=packed.device)
    
    # Unpack: lower 4 bits = even indices, upper 4 bits = odd indices
    lower = (packed & 0x0F).to(torch.int32)
    upper = ((packed >> 4) & 0x0F).to(torch.int32)
    del packed  # Free memory immediately
...
def test_paged_attention(
    ...
    if quant_cache_dtype is None:
        ...
        if kv_cache_dtype == "fp8":
            ...
            # Use unpacked dequant since we now use unpacked FP4 format
            sample_blocks = min(1000, num_blocks)
            k_dequant_sample = dequant_fp4_unpacked_to_float(k_quant_flat[:sample_blocks], k_scale_reshaped[:sample_blocks].unsqueeze(-1), target_dtype=torch.float32)
            v_dequant_sample = dequant_fp4_unpacked_to_float(v_quant_flat[:sample_blocks], v_scale_reshaped[:sample_blocks].unsqueeze(-1), target_dtype=torch.float32)
            ...
            print(f"  [FP4] K cache MAE: {k_err:.6f}, Relative: {k_rel_err*100:.2f}%")
            print(f"  [FP4] V cache MAE: {v_err:.6f}, Relative: {v_rel_err*100:.2f}%")
            ...
            if USE_FP4_KERNEL:
                try:
                    ...
                    # Use mean of per-token scales as per-tensor scale (approximation)
                    k_scale_run = k_scale_quant.mean().reshape(1)
                    v_scale_run = v_scale_quant.mean().reshape(1)
                    kv_dtype_str = "fp4"
                    print(f"  [FP4] Using FP4 kernel with per-tensor scale approximation")
                except Exception as e:
                    print(f"  [FP4] Kernel failed ({e}), falling back to Python dequant...")
                    USE_FP4_KERNEL = False
            
            if not USE_FP4_KERNEL:
                # Use Python dequantization (verified to work)
                print(f"  [FP4] Using Python dequantization (kernel experimental)")
                k_quant_flat = rearrange(key_cache_quant, "b h d1 s d2 -> b h s (d1 d2)")
                v_quant_flat = rearrange(value_cache_quant, "b h d s -> b h s d")
                
                k_dequant = dequant_fp4_to_float(k_quant_flat, k_scale_reshaped.unsqueeze(-1), target_dtype=dtype)
                del k_quant_flat
                torch.cuda.empty_cache()
                v_dequant = dequant_fp4_to_float(v_quant_flat, v_scale_reshaped.unsqueeze(-1), target_dtype=dtype)
                del v_quant_flat
                torch.cuda.empty_cache()
                
                key_cache_run = k_dequant
                value_cache_run = v_dequant
        ...
        acc = checkAllclose(out_golden, out_aiter, msg=f"golden vs aiter:{time_aiter:.2f} us")
