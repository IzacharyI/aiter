...
from aiter.ops.flydsl.kernels.mega_moe_gemm1 import GateMode, compile_fused_moe_gemm1
from aiter.ops.flydsl.kernels.mixed_moe_gemm_2stage import compile_mixed_moe_gemm1, compile_mixed_moe_gemm2
...
try:
    import aiter
    from aiter.utility import dtypes as _adt
...
_MEGA_TUNING_DIR = Path(__file__).resolve().parent / "kernels" / "mega_moe_tuning_config"
    ...
    def __init__(
        ...
        *,
        ...
        if self._fp8_cast:
            ...
            self._out_dtype_str = "bf16" if comb_cfg.combine_dtype == torch.bfloat16 else "f16"
...
def _resolve_stage1_config(*, model_dim, inter_dim, experts, topk, quant,
    ...
    # INTERLEAVE (g1u1) halves tile_n in the c_shuffle epilog (_gui_tile_n = tile_n//2). A tuned
    # tile_n=128 (fine for SEPARATED) leaves the interleave epilog only 64 wide -> ~15% slower.
    # Bump to 256 (epilog 128 wide) when inter_dim allows; SEPARATED keeps the tuned value.
    ...
    if xcd > 0:
        try:
            ...
            _cu_g = 256
        # gx grid over the N=2*inter output: SEPARATED splits gate/up (2 B-tile streams) so
        # gx=ceil(inter/tile_n); INTERLEAVE (g1u1) is a single stream over 2*inter so gx=ceil(2*inter/tile_n).
        if gate_mode is GateMode.INTERLEAVE:
            _gx_g = max(1, (inter_dim * 2 + int(tile_n) - 1) // int(tile_n))
        else:
            _gx_g = max(1, (inter_dim * 2 + 2 * int(tile_n) - 1) // int(tile_n) // 2)
        # XCD swizzle maps gx N-tiles onto the 8-XCD group: valid when gx cleanly relates to 8
        # (divisor OR multiple -> whole rounds), not only gx<=8. INTERLEAVE's single 2*inter stream
        # gives gx=16 (tn256): a multiple of 8, so the swizzle still aligns (2 rounds) -> keep it.
        _gx_xcd_ok = (_cu_g % _gx_g == 0) and (8 % _gx_g == 0 or _gx_g % 8 == 0)
        if not _gx_xcd_ok:
            import warnings
            warnings.warn(
...
def _default_stage1_b_nt(mtpr: int) -> int:
    """B-load cache_modifier heuristic for the fused stage-1 GEMM1 (ported from FlyDSL
    deepseek_dispatch_gemm1_swiglu §13.4.17): decode (mtpr<=128, W not reused across an
    expert's M-tiles) streams the B/weight load (3 = GLC|SLC, bypass L2); prefill (W reused
    across M-tiles) caches (0). Override with env AITER_MEGA_S1_BNT."""
    _env = os.environ.get("AITER_MEGA_S1_BNT", "").strip()
    if _env:
        return int(_env)
    return 3 if int(mtpr) <= 128 else 0
...
class MegaMoeStage1:
    ...
    def __init__(self, *, comm_op, cfg, rank, world_size, model_dim, inter_dim,
                 ...
                 gate_mode=GateMode.SEPARATED, act_clamp=0.0):
        assert quant in ("a4w4", "a8w4")
        ...
        self.act_clamp = float(act_clamp)
        ...
        # b_nt (B-load cache_modifier) heuristic: streams the weight load for decode-size
        # batches (mtpr<=128), caches for prefill. Single compile-time value per op (mtpr is
        # fixed per op), so no runtime variant switch is needed.
        self._b_nt = _default_stage1_b_nt(self.mtpr)

        # §13.4.13 payload dedup (ported from FlyDSL deepseek_dispatch_gemm1_swiglu): send each
        # token's activation ONCE per dest PE into a token-major rx_tok buffer; same-dest sibling
        # routes skip the ~7KB xGMI copy and the GEMM gathers A by src_global. atom_contract +
        # non-compact only (the compact prologue keeps the baseline expert-major write). Enabled by
        ...
        if os.environ.get("AITER_MEGA_DEBUG_OPT", "0") == "1" and self.rank == 0:
            print(f"[megaopt] rank0 mtpr={self.mtpr} compact={self.compact} "
                  ...
                  f"dedup={self._dedup} (AITER_MEGA_S1_DEDUP={_dd_env!r})", flush=True)

        # Compile the megakernel (serialize across ranks to bound peak memory).
        # Shared kwargs so the base and (optional) streaming b_nt variants stay byte-identical
        # except for the B-load cache_modifier.
        _mega_kwargs = dict(
            ...
            atom_contract=self.atom_contract, dedup=self._dedup,
        ...
        def _compile_variant(b_nt_val):
            return compile_fused_moe_gemm1(b_nt=b_nt_val, **_mega_kwargs)

        # b_nt runtime variant selection (ported from FlyDSL deepseek_dispatch_gemm1_swiglu
        # §13.4.17 candidate: it compiles BOTH the base (b_nt=self._b_nt) and the streaming
        # (b_nt=3, GLC|SLC L2-bypass) kernels and picks per-forward by cur_tok, so decode-size
        # batches get the streaming B-load even when the op's fixed mtpr>128 would otherwise
        # lock b_nt to the prefill/cache value. Disable with AITER_MEGA_S1_BNT_STREAM=0;
        # override the switch point with AITER_MEGA_S1_BNT_DECODE_TOK (default 128).
        self._bnt_stream = (os.environ.get("AITER_MEGA_S1_BNT_STREAM", "1").strip() != "0")
        self._bnt_decode_tok = int(os.environ.get("AITER_MEGA_S1_BNT_DECODE_TOK", "128"))
        self._b_nt_stream = 3
        ...
        for pe in range(world_size):
            if rank == pe:
                self.mega = _compile_variant(self._b_nt)
            if dist.is_initialized():
                dist.barrier()
        # Streaming variant only when it actually differs from the base value.
        if self._bnt_stream and self._b_nt_stream != self._b_nt:
            for pe in range(world_size):
                if rank == pe:
                    self.mega_nt = _compile_variant(self._b_nt_stream)
                if dist.is_initialized():
                    dist.barrier()
        if os.environ.get("AITER_MEGA_DEBUG_OPT", "0") == "1" and self.rank == 0:
            print(f"[megaopt] rank0 b_nt_base={self._b_nt} b_nt_stream="
                  f"{self._b_nt_stream if self.mega_nt is not None else None} "
                  f"decode_tok<={self._bnt_decode_tok}", flush=True)

        # Views into op buffers (A-input + raw A-scale + metadata the GEMM consumes).
        v = self.op._ll_views()
        self._rx = v["rx_em"]
        # §13.4.13 payload dedup: symmetric token-major rx_tok (indexed by global src token id).
        # Sized like rx_em (nvm rows >= npes*mtpr, so any src_global gather stays in-bounds); the
        # GEMM reads it in place of rx_em. p2p_rx_tok is appended to the dispatch table at slot 26.
    ...
    def _build_disp_table(self):
        ...
        if self.scheme == "fixedslot":
            ...
            if self.compact:
                ...
                if self._dedup:
                    tbl.append(self.p2p_rx_tok.data_ptr())               # 26 §13.4.13 p2p_rx_tok
    ...
    def forward(self, x, wts, scales, topk_ids, stream=None) -> "Stage1Output":
        ...
        # b_nt runtime variant: decode-size batches stream the B/weight load (b_nt=3, L2 bypass),
        # prefill keeps the base cache modifier. Falls back to base when no streaming variant built.
        _kern = (self.mega_nt if (self.mega_nt is not None
                                  and cur_tok <= self._bnt_decode_tok) else self.mega)
        _kern(self._out, a_mat, self.w1, self._scale_i32, self.w1_scale,
...
class MegaMoE:
    ...
    def __init__(
        ...
        *,
        ...
        self.act_clamp = float(act_clamp)
        # gate-up mode for the fused stage-1 GEMM1: default INTERLEAVE(g1u1) for a8w4 (mirrors
        # aiter's a_dtype=="fp8" gating), SEPARATED for a4w4. Overridable; the caller's w1/w1_scale
        # shuffle MUST match (INTERLEAVE -> shuffle_weight_w4/shuffle_scale_w4 gate_up=True).
        self.gate_mode = ((GateMode.INTERLEAVE if quant == "a8w4" else GateMode.SEPARATED)
                          if gate_mode is None else GateMode(gate_mode))
        ...
        self._s1cfg = _resolve_stage1_config(
            ...
            use_async_copy=bool(use_async_copy), gate_mode=self.gate_mode)
        ...
        # dispatch_dtype/scale reflect the ACTIVE dispatch path: fused stage-1 uses
        # the group-major (expert-major) quantized dispatch (fp8/fp4 + scales); the
        # non-fused path uses the standard token-major bf16 dispatch (no scales).
        _gm_on = self.enable_fused_stage1
        self.comb_cfg = FlyDSLDispatchCombineConfig(
            ...
            dispatch_dtype=(self._s1cfg["data_type"] if _gm_on else torch.bfloat16),
            scale_dim=(self._s1cfg["scale_dim"] if _gm_on else 0),
            scale_type_size=(1 if _gm_on else 0),
            enable_std_moe=False,
            enable_group_major=_gm_on,
        ...
        # sequentially (also under CUDA-graph replay), so the transient workspace
        # buffers are safe to reuse — mirrors DeepGEMM's single shared symm buffer.
        if shared_comb_op is not None:
            self.comb_op = shared_comb_op
        else:
        ...
        if self.enable_fused_stage1:
            ...
            self.stage1 = MegaMoeStage1(
                ...
                gate_mode=self.gate_mode, act_clamp=self.act_clamp)
    ...
    def _build_nonfused_stage1(self, w1, w1_scale):
        ...
        # INTERLEAVE (g1u1) halves tile_n in the c_shuffle epilog (_gui_tile_n = tile_n//2), so
        # tile_n=256 gives a full 128-wide epilog (matches SEPARATED's tn128) and is ~15% faster
        # than tn128 here; SEPARATED keeps tn128. Guard on inter_dim divisibility.
        tm, tk = 32, 256
        ...
        self._nf_gemm1 = compile_mixed_moe_gemm1(
            ...
            # w1/w1_scale layout follows the op's gate_mode (INTERLEAVE g1u1 for a8w4);
            # without this the gemm1 defaults to SEPARATED and misreads interleaved w1.
            gate_mode=self.gate_mode)
    ...
    def _run_stage2(self, s1, run_tokens, stream, slice_output):
        ...
        if out_tok is None:
            cfg = self.comb_cfg
            out_tok = (self.comb_op.shmem_comb_out_tok.view(torch.int8)[: self.mtpr * cfg.combine_token_bytes]
                       .view(cfg.combine_dtype).view(self.mtpr, cfg.combine_token_view_dim))
