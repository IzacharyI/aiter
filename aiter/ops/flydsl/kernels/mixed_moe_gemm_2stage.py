...
def _is_index_value(value):
    raw = _raw_value(value)
    return hasattr(raw, "type") and isinstance(raw.type, ir.IndexType)


def _shrui_compat(lhs, rhs, *args, **kwargs):
    lhs_raw = _raw_value(lhs)
    rhs_raw = _raw_value(rhs)
    if _is_index_value(lhs_raw) and not _is_index_value(rhs_raw):
        lhs_raw = arith.index_cast(T.i32, lhs_raw)
    elif _is_index_value(rhs_raw) and not _is_index_value(lhs_raw):
        rhs_raw = arith.index_cast(T.i32, rhs_raw)
    return _orig_shrui(lhs_raw, rhs_raw, *args, **kwargs)


arith.shrui = _shrui_compat


def _i32_if_index(value):
    raw = value.ir_value() if hasattr(value, "ir_value") else value
    if hasattr(raw, "type") and str(raw.type) != "i32":
        ...
        def moe_gemm1(
            ...
            def _moe_gemm1_body():
                ...
                for i in range_constexpr(num_x_loads):
                    ...
                    s_i32 = arith.shrui(
                        _i32_if_index(fused_i),
                        _i32_if_index(arith.constant(24, type=T.i32)),
                    )
                ...
                def _rearrange_a_scale(raw_i32):
                    ...
                    if const_expr(pack_M >= scale_mn_pack):
                        return raw_i32
                    b_k0 = arith.andi(
                        arith.shrui(_i32_if_index(raw_i32), _i32_if_index(_scale_shift)), _scale_mask_lo
                    )
                    b_k1 = arith.andi(
                        arith.shrui(_i32_if_index(raw_i32), _i32_if_index(_scale_shift_hi)), _scale_mask_lo
                    )
                    return arith.ori(
                ...
                def _rearrange_b_scale(raw_i32):
                    ...
                    if const_expr(pack_N >= scale_mn_pack):
                        return raw_i32
                    b_k0 = arith.andi(
                        arith.shrui(_i32_if_index(raw_i32), _i32_if_index(_bscale_shift)), _scale_mask_lo
                    )
                    b_k1 = arith.andi(
                        arith.shrui(_i32_if_index(raw_i32), _i32_if_index(_bscale_shift_hi)), _scale_mask_lo
                    )
                    return arith.ori(
                        b_k0, arith.shli(b_k1, arith.constant(8, type=T.i32))
                    )
            ...
            def _moe_gemm2_then_body():
                ...
                def _apply_k_shift(scale_vec, k_shift_bits):
                    if const_expr(k_shift_bits > 0):
                        ...
                        val = _i32_if_index(val)
                        val = arith.shrui(
                            _i32_if_index(val),
                            _i32_if_index(arith.constant(k_shift_bits, type=T.i32)),
                        )
                    ...
                    *,
                    ...
                    if const_expr(use_mfma32_k64):
                        ...
                        def preselect_mfma32_scale_byte(raw_i32, scale_k_half: int):
                            ...
                            byte = arith.andi(
                                arith.shrui(_i32_if_index(raw_i32), _i32_if_index(shift)),
                    ...
                    for k_idx in range_constexpr(ku_count):
                        ...
                        for mi in range_constexpr(m_repeat_packed):
                            ...
                            a_scale_val = _i32_if_index(a_scale_val)
                            if const_expr(_m_scale_shift_i32 is not None):
                                a_scale_val = arith.shrui(
                                    _i32_if_index(a_scale_val), _i32_if_index(_m_scale_shift_i32)
                                )
                            for ni in range_constexpr(num_acc_n_packed):
                                ...
                                b_scale_val = _i32_if_index(b_scale_val)
                                if const_expr(_n_scale_shift_i32 is not None):
                                    b_scale_val = arith.shrui(
                                        _i32_if_index(b_scale_val), _i32_if_index(_n_scale_shift_i32)
                                    )
