    ...
    if m and m.group(0):
        ...
        if params is not None:
            ...
            if m.group("am") is not None:
                # Approach A: `_am{N}` merges N consecutive sort blocks into one
                # compute tile (weight-reuse across more token rows).
                extra["a_merge"] = int(m.group("am"))
...
def flydsl_moe_stage1(
    ...
    *,
    ...
    # tile, weight loaded once; solo -> 64 rows).  Requires a_merge=2 (tile_m*2).
    ...
    if int(a_merge) > 1 and os.environ.get("AITER_S1_NOBMAP", "0") != "1":
        blk_map = True
    if blk_map:
        a_merge = 2
